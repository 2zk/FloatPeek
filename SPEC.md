...
- 複数のフォルダタブと選択中タブの保存・読み込み
- 縦方向のタブ一覧による表示フォルダ切り替え
- 設定画面でのタブ追加・削除・名称・フォルダ設定・再読み込み
...
フォルダ選択、タブの追加・削除・名称変更、手動再読み込みはメイン画面に置かず、設定画面に集約する。
...
| 状態 | 表示 |
| --- | --- |
| タブ未登録 | `No tabs configured` / `Add a tab in Settings.` |
| 選択中タブのフォルダ未設定 | `No folder selected` / `Choose a folder for this tab in Settings.` |
...
タブ未登録の場合はタブ追加を、選択中タブのフォルダ未設定の場合は設定画面でのフォルダ選択を促す状態メッセージを表示する。
...
- ファイル削除、リネーム、移動などのファイル変更機能は実装しない
