...
- ファイル名、追加日、変更日によるソート
- 単一選択、トグル選択、範囲選択
- 既定アプリで開く、名前変更、Quick Look、コピー、パスコピー、Finder 表示
- ゴミ箱への移動
...
タイルのダブルクリック、またはコンテキストメニューの `Open` で macOS の既定アプリを開く。
...
単一選択時に `Return` またはテンキー `Enter` を押すと、選択タイルのファイル名を編集する。拡張子を除く部分だけを全選択し、拡張子は変更対象にしない。複数選択時は編集を開始しない。

編集状態では `Return` で確定、`Escape` で取消、フォーカス移動で確定する。編集中のキー入力は一覧操作として処理しない。変更後は新しい URL を選択状態に保持し、現在の並び順を再適用する。
...
| `Escape` | ウィンドウを非表示 |
| `Return` / テンキー `Enter` | 単一選択ファイルの名前を変更 |
